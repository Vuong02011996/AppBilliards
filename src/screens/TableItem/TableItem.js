...
function TableItem({ route, navigation }) {
        ...
        async function handleThayDoiGioNghi() {
            ...
            if (valueGioVao != 'Chưa đặt') {
                ...
                if (intervalTimeCost > 0) {
                    ...
                    Alert.alert('Giờ vào đặt sau giờ nghỉ!, Vui lòng đặt giờ vào trước rồi mới bấm giờ nghỉ!');
                }
            } else {
    ...
    const handelChangeTienMoiPhut = (newTienGioMoiPhut) => {
        ...
        const valueThoiGianChoi = readFieldDataDB('Danh sach ban', title, 'gioNghi');
    ...
    const getData = () => {
        const itemTable = [
            { name: 'Item A', quantity: 2, unit: 'pcs', price: 10, amount: 20 },
            { name: 'Item B', quantity: 1, unit: 'kg', price: 15, amount: 15 },
            { name: 'Item C', quantity: 3, unit: 'pcs', price: 5, amount: 15 },
        ];

        const timeCharge = { start: '08:00', end: '10:00', minutes: 120, price: 5, amount: 10 };
        ...
        const invoiceText = `
          ...
          ${itemTable
              .map((item) => `| ${item.name} | ${item.quantity} | ${item.unit} | ${item.price} | ${item.amount} |`)
              .join('\n')}
          
          TIME CHARGE
          -----------
          | Start Time | End Time | Minutes | Price | Amount |
          | ${timeCharge.start} | ${timeCharge.end} | ${timeCharge.minutes} | ${timeCharge.price} | ${
            timeCharge.amount
        } |
    ...
    const handelInHoaDon = () => {
        ...
        // const xmlDataWithCut = data + cutCommand;
                ...
                <View style={{ marginHorizontal: 8 }}>
                    ...
                    <View>
                        ...
                        <View style={{ flexDirection: 'row' }}>
                            ...
                            <Text
                                style={[styles.textThoiGianChoi, { minWidth: 246, color: '#000', fontWeight: '500' }]}
                            >
                                K
                            </Text>
                        </View>
