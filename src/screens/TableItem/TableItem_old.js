...
function TableItem({route, navigation}) {
  ...
  const [gioNghi, setGioNghi] = React.useState('Chưa thanh toán');
    ...
    getDataObject(KEY_TABLE_ITEM).then(infoTable => {
      for (var i = 0; i < infoTable.length; i++) {
        ...
        if (infoTable[i].tableID == tableId) {
          ...
          var menuMoney = infoTable[i].infoMenu.reduce(function (
            total,
            infoMenuItem,
          ) {
      ...
      getDataObject(KEY_TABLE_ITEM).then(infoTable => {
        for (var i = 0; i < infoTable.length; i++) {
          ...
          if (infoTable[i].tableID == tableId) {
            ...
            infoTable[i].gioNghi = 'Chưa thanh toán';
            break;
          }
        }
        storeObjectValue(KEY_TABLE_ITEM, infoTable);
  ...
  const handelChangeTienMoiPhut = newTienGioMoiPhut => {
    getDataObject(KEY_TABLE_ITEM).then(infoTable => {
      for (var i = 0; i < infoTable.length; i++) {
        ...
        if (infoTable[i].tableID == tableId) {
          if (
            infoTable[i].gioVao != 'Chưa đặt' &&
            infoTable[i].gioNghi != 'Chưa đặt'
          ) {
            const moneyTime =
              (infoTable[i].thoiGianChoi * newTienGioMoiPhut) / 1000;
            const totalMoney = moneyTime + tienMenu;
            setTienGio(moneyTime);
            setTongTien(totalMoney);
          }
    ...
    getDataObject(KEY_TABLE_ITEM).then(infoTable => {
      ...
      if (infoTable === null) {
        ...
        if (tableData === undefined) {
          ...
          storeObjectValue(KEY_TABLE_ITEM, infoTable);
          console.log(
            'infoTable TH2.1 infoTable != null, tableData = undefined, infoTable:',
            infoTable,
          );
        } else {
          let menuMoney = 0;
